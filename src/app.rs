...
fn CircuitPage() -> impl IntoView {
    ...
    let selected_signal = move || {
        SignalType::from_id(&selected_signal_id.get())
            .unwrap_or(SignalType::Constant)
    };

    let (circuit_values, set_circuit_values) =
    signal(HashMap::<String, String>::new());
    
    let (signal_values, set_signal_values) =
    signal(HashMap::<String, String>::new());

    let (simulation_preview, set_simulation_preview) =
    signal(Option::<SimulationPreview>::None);

    let (calculation_error, set_calculation_error) =
